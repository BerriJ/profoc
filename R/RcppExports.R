...
#' @export
batch <- function(y, experts, tau = as.numeric( c()), affine = FALSE, positive = FALSE, intercept = FALSE, debias = TRUE, initial_window = 30L, expanding_window = TRUE, loss_function = "quantile", loss_parameter = 1, lambda = as.numeric( c(-1 / 0)), forget = as.numeric( c(0)), forget_performance = 0, fixed_share = as.numeric( c(0)), ndiff = as.numeric( c(1.5)), deg = as.numeric( c(3)), knot_distance = as.numeric( c(0.025)), knot_distance_power = as.numeric( c(1)), trace = TRUE, lead_time = 0L, allow_quantile_crossing = FALSE, soft_threshold = as.numeric( c(-1 / 0)), hard_threshold = as.numeric( c(-1 / 0))) {
    .Call(`_profoc_batch`, y, experts, tau, affine, positive, intercept, debias, initial_window, expanding_window, loss_function, loss_parameter, lambda, forget, forget_performance, fixed_share, ndiff, deg, knot_distance, knot_distance_power, trace, lead_time, allow_quantile_crossing, soft_threshold, hard_threshold)
}
...
#' allow_quantile_crossing = FALSE, soft_threshold = -Inf, hard_threshold = -Inf)
#' @export
batch_basis <- function(y, experts, tau = as.numeric( c()), affine = FALSE, positive = FALSE, intercept = FALSE, debias = TRUE, initial_window = 30L, expanding_window = TRUE, loss_function = "quantile", loss_parameter = 1, lambda = as.numeric( c(-1 / 0)), forget = as.numeric( c(0)), forget_performance = 0, fixed_share = as.numeric( c(0)), ndiff = as.numeric( c(1.5)), deg = as.numeric( c(3)), knot_distance = as.numeric( c(0.1)), knot_distance_power = as.numeric( c(1)), trace = TRUE, lead_time = 0L, allow_quantile_crossing = FALSE, soft_threshold = as.numeric( c(-1 / 0)), hard_threshold = as.numeric( c(-1 / 0))) {
    .Call(`_profoc_batch_basis`, y, experts, tau, affine, positive, intercept, debias, initial_window, expanding_window, loss_function, loss_parameter, lambda, forget, forget_performance, fixed_share, ndiff, deg, knot_distance, knot_distance_power, trace, lead_time, allow_quantile_crossing, soft_threshold, hard_threshold)
}
...
#' @export
online <- function(y, experts, tau = as.numeric( c()), intercept = FALSE, loss_function = "quantile", loss_parameter = 1, ex_post_smooth = FALSE, ex_post_fs = FALSE, lambda = as.numeric( c(-1 / 0)), method = "boa", method_var = "A", forget_regret = as.numeric( c(0)), forget_performance = 0, fixed_share = as.numeric( c(0)), gamma = as.numeric( c(1)), ndiff = as.numeric( c(1.5)), deg = as.numeric( c(3)), knot_distance = as.numeric( c(0.1)), knot_distance_power = as.numeric( c(1)), gradient = TRUE, loss_array = as.numeric( c()), regret_array = as.numeric( c()), trace = TRUE, init_weights = NULL, lead_time = 0L, allow_quantile_crossing = FALSE, soft_threshold = as.numeric( c(-1 / 0)), ex_post_soft_threshold = FALSE, hard_threshold = as.numeric( c(-1 / 0)), ex_post_hard_threshold = FALSE) {
