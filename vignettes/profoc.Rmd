...
The code above will compute the CRPS Learning algorithm described in [@berrisch2021crps]. This algorithm is based on Bernstein online aggregation [@wintenberger2017optimal] and uses the quantile loss to compute weights on the probability grid. Other algorithms like `ewa` or `ml-poly` can be computed by setting the `method` argument accordingly.  
...
These are averages over time, over the probability grid, and possibly also over all marginals. The losses of the Experts and the Forecaster can be accessed via `combination$experts_loss` and `combination$forecaster_loss` respectively.

We continue by inspecting the weights which were assigned during the online learning process:

```{r}
dim(combination$weights)
```

The most recent weights can be plotted using `autoplot()` if `ggplot2` is available.

```{r, echo = TRUE, out.width='100%', fig.width=7, fig.height = 4, dpi = 300}
autoplot(combination)
```

We see that expert one receveives high weights, in particular in the right tail. This is because the predictions of expert one are closer to the true DGP, especially in the right tail. 

We offer `tidy()` methods for the most important objects. These methods return a `tibble` which can be used for further analysis. So we can use `tidy()` to extract the weights of the algorithm:

```{r}
tidy(combination$weights)
```

The `tibble` above is in the long format. So it is well prepared for further analysis using `ggplot2`, `dplyr`, etc..
